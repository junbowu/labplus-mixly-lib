/*-------------------------------------------------------------------------
  ...
  LED devices such as Labplus FLORA RGB Smart Pixels and NeoPixel strips.
  ...
  Written by Phil Burgess / Paint Your Dragon for Labplus Industries,
  contributions by PJRC, Michael Miller and other members of the open
  source community.

  Labplus invests time and resources providing this open source code,
  please support Labplus and open-source hardware by purchasing products
  from Labplus!

  -------------------------------------------------------------------------
  This file is part of the Labplus NeoPixel library.
...
#include "Labplus_NeoPixel.h"

// Constructor when length, pin and type are known at compile-time:
Labplus_NeoPixel::Labplus_NeoPixel(uint16_t n, uint8_t p, neoPixelType t) :
...
Labplus_NeoPixel::Labplus_NeoPixel() :
...
void Labplus_NeoPixel::begin(void) {
  if(pin >= 0) {
    pinMode(pin, OUTPUT);
...
void Labplus_NeoPixel::updateLength(uint16_t n) {
...
void Labplus_NeoPixel::updateType(neoPixelType t) {
...
void Labplus_NeoPixel::show(void) {
...
// 8 MHz(ish) AVR ---------------------------------------------------------
  ...
  if(is800KHz) {
    ...
    // this covers all the pins on the Labplus Flora and the bulk of
...
void Labplus_NeoPixel::setPin(uint8_t p) {
...
void Labplus_NeoPixel::setPixelColor(
...
void Labplus_NeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t *p,
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t Labplus_NeoPixel::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Convert separate R,G,B,W into packed 32-bit WRGB color.
// Packed format is always WRGB, regardless of LED strand color order.
uint32_t Labplus_NeoPixel::Color(uint8_t r, uint8_t g, uint8_t b, uint8_t w) {
  return ((uint32_t)w << 24) | ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t Labplus_NeoPixel::getPixelColor(uint16_t n) const {
...
uint8_t *Labplus_NeoPixel::getPixels(void) const {
  return pixels;
}

uint16_t Labplus_NeoPixel::numPixels(void) const {
  return numLEDs;
}
...
void Labplus_NeoPixel::setBrightness(uint8_t b) {
...
uint8_t Labplus_NeoPixel::getBrightness(void) const {
  return brightness - 1;
}

void Labplus_NeoPixel::clear() {
  memset(pixels, 0, numBytes);
}
